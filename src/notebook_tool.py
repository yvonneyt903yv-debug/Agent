...
class NotebookSkill:
    ...
    async def generate_podcast(self, notebook_id, instructions="生成一个中文播客生成一个中文播客15分钟以内", timeout=600):
        """生成播客音频"""
        if hasattr(notebook_id, 'id'):
            notebook_id = notebook_id.id
        notebook_id = str(notebook_id)
        ...
        try:
            ...
            # 下载播客（使用绝对路径）
            output_dir = os.path.join(OUTPUT_DIR, "podcasts")
